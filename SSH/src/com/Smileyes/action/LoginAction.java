...
public class LoginAction extends ActionSupport implements ModelDriven<Employee> {
	...
	public String login() {
		...
		if (noLogin) {
			...
			Map<String, Object> request = ServletActionContext.getContext()
					.getContextMap();
			request.put("list", this.employeeService.getAll());// 保存列表
	...
	public String update() {
		Employee oldEmp = this.employeeService.findById(employee.getId());// 旧的对象,这里用到了Hibernate的一级缓存
	...
	public String remove() {
		...
		if (employee.getId() != id) {
			this.employeeService.remove(employee.getId());
